...
async function readData(file, pos, result) {
    ...
    for (let chunk of file.chunks) {
        if (chunk.start <= pos && pos < chunk.end) {
            ...
            if (remaining > 0) {
                await readData(file, available, result.subarray(available - pos))
            }
    ...
    readData(file, pos, result)
}

// vfs.jsFullPathname = async function (pVfs, zPath, nPathOut, zPathOut) {
...
vfs.jsRead = async function (pFile, zBuf, iAmt, iOfst) {
    const result = vfs.HEAPU8.subarray(zBuf, zBuf + iAmt)
    const arr = await readData(files[pFile], iOfst, result)
    console.log('read', iOfst, iAmt, result)
    // vfs.HEAPU8.set(arr, zBuf)
    return 0
}
// vfs.jsFileControl = async function (pFile, op, pArg) {
//     console.log('jsFileControl', pFile, op, pArg)
...
//     return 0
// }
// vfs.jsLock = async function (pFile, eLock) {
//     console.log('jsLock', pFile, eLock)
//     return 0
// }
// vfs.jsUnlock = async function (pFile, eLock) {
//     console.log('jsUnlock', pFile, eLock)
//     return 0
// }
...
// vfs.jsCheckReservedLock = async function (pFile, pResOut) {
//     console.log('jsCheckReservedLock', pFile, vfs.getValue(pResOut, 'i32'))
//     return 0
// }
// vfs.jsRandomness = async function (pVfs, nByte, zByte) {
//     console.log('jsRandomness', pVfs, nByte, zByte)
...
vfs.jsFileSize = async function (pFile, pSize) {
    ...
    console.log('File Size', file.url, size)

    return 0
}
// vfs.jsDelete = async function (pVfs, zPath, dirSync) {
//     console.log('jsDelete', pVfs, vfs.UTF8ToString(zPath), dirSync)
//     return 0
// }
// vfs.jsWrite = async function (pFile, zBuf, iAmt, iOfst) {
//     console.log('jsWrite', pFile, zBuf, iAmt, iOfst)
//     return 0
// }
// vfs.jsSync = async function (pFile, flags) {
//     console.log('jsSync', pFile, flags)
//     return 0
// }
...
vfs.onRuntimeInitialized = async function () {
    ...
    await exec(`SELECT * FROM covid.economist_excess_deaths order by rowid LIMIT 10`)
